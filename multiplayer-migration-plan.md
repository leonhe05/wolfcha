...
当前前端 `useGameLogic.ts` 中阶段流转是**命令式**的：业务代码显式调用 `transitionPhase(state, "XXX")` 指定下一个阶段，条件分支（守卫死了跳过、女巫没药了跳过等）散落在各处 `if-else` 里。

后端重构时应改为**声明式状态机**：

1. **阶段流转图集中配置**：所有 `Phase → Phase` 的流转规则写在一张配置表里，附带 `condition(gameState)` 条件函数用于分支判断（如 NIGHT_START 后先尝试进入守卫阶段，没守卫则自动进入狼人阶段）。
2. **自动推进**：业务代码（处理玩家操作、AI 行动）只负责返回 `"阶段已完成"`，由状态机根据流转图自动寻找下一个合法阶段，不暴露目标阶段名给业务层。
3. **生命周期钩子**：每个阶段统一提供 `onEnter`（初始化、发系统消息）和 `onExit`（结算、清理），避免逻辑散落在业务代码中。
4. **事件驱动**：状态机内部完成流转后 emit `PHASE_CHANGED` 事件，WebSocket 广播层订阅该事件自动向客户端推送 `STATE_UPDATE`。

核心原则：业务代码只处理"当前阶段内发生了什么"，状态机负责"现在该去哪"。
